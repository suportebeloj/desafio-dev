...
const marketBalance = `-- name: MarketBalance :one
SELECT CAST(sum(
               CASE
...
func (q *Queries) MarketBalance(ctx context.Context, market string) (float64, error) {
	row := q.db.QueryRowContext(ctx, marketBalance, market)
